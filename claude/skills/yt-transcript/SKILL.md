...
1. Fetch the transcript (skip if `./transcripts/<video_id>.txt` already exists):
   ```
   uv run ~/.claude/skills/yt-transcript/scripts/fetch_transcript.py <url_or_video_id>
   ```
   Writes `./transcripts/<video_id>.txt` in the current working directory as timestamped
   `HH:MM:SS  text` lines. Pulls auto-generated or manual English captions via `yt-dlp`
   (`--lang` to change). The script declares its own dependencies (PEP 723), so `uv run`
   handles it standalone — no project setup needed, just `uv` and `yt-dlp` on PATH via uv's
   ephemeral env.

2. Search it for the relevant topic:
   ```
   uv run ~/.claude/skills/yt-transcript/scripts/search_transcript.py ./transcripts/<video_id>.txt "<regex>" -C 1
   ```
